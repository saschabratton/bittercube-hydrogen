 ...
 * @param {*} obj An array of graphql collection objectgs
 * @param {*} type either "Artist" or "Museum"
 * @returns
 */
export function filterCollectionByType(obj, type) {
  const output = obj.filter(item => item.type?.value === type)
  return output
}


export function extractProductCollectionByType(product, type="Artist") {
  return product.collections ? product.collections.nodes.find((c) => c.type?.value === type) : null
}


/**
 * filterInactiveProducts
 * @param {*} variants An array of all variant objects
 * @returns Filters out all expired or upcoming variants and returns a single variant
 */
export function filterInactiveProducts(products) {

  const now = getUnixTime(new Date())
  const filteredProducts = products.filter(product => {

    if (!product.endDate || !product.startDate || !product.rows || !product.cols) { return false } // Must have tile data, start date and end date
    if (
      getUnixTime(new Date(product.endDate?.value)) >= now // end date has not passed
      && getUnixTime(new Date(product.startDate?.value)) <= now // start date has passed
      && product.totalInventory > 0 // there are available tiles left
    ) {
      return product
    }

  })
  // If there are still multiple variants, return the first one.
  // return filteredVariants.length ? filteredVariants.filter((variant,i) => i == 0) : null

  // For development, comment out the return above to return ALL of the variants.
